...
const Field=({label,children,className=""}:{label:string;children:React.ReactNode;className?:string})=><div className={`space-y-2 ${className}`}><Label>{label}</Label>{children}</div>
 ...
 const submit=async(event:React.FormEvent)=>{
   ...
   if(!form.title.trim()||!form.address.trim()||!form.city.trim())return toast({title:"Missing building information",description:"Building name, address, and city are required.",variant:"destructive"});
   setSaving(true);
   try{
     ...
     const images=await uploadPropertyImages(files,user.id,`buildings/${Date.now()}`);
