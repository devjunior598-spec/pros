...
export function SinglePropertyForm() {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (!form.title.trim() || !form.address.trim() || !form.city.trim() || Number(form.rent) <= 0) return toast({title:"Check required fields",description:"Property name, rent, address, and city are required.",variant:"destructive"})
    setSaving(true)
    try {
    ...
    } catch(error){toast({title:"Could not publish property",description:error instanceof Error?error.message:"Please try again.",variant:"destructive"})} finally{setSaving(false)}
  }
  ...
  return <form onSubmit={submit} className="mx-auto max-w-5xl space-y-6 pb-24">
    ...
    <div className="sticky bottom-4 flex justify-end rounded-xl border bg-white/95 p-3 shadow-lg backdrop-blur"><Button size="lg" disabled={saving}>{saving?<Loader2 className="mr-2 h-4 w-4 animate-spin"/>:<Check className="mr-2 h-4 w-4"/>}{saving?"Publishing…":"Publish property"}</Button></div>
  </form>
}
